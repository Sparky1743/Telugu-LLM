...
def extract_data_from_html(html_content):
    ...
    try:
        ...
        # Find all p tags within the div with class "story_content details-content-story"
        story_content = soup.find('div', class_='story_content details-content-story')
        if story_content:
            p_tags = story_content.find_all('p')
            for p in p_tags:
                # Check if the p tag contains any Telugu text
                if is_telugu(p.text):
                    extracted_data.append(p.text.strip())
    except Exception as e:
...
csv_folder_path = '/home/llmtelugu/csv_data/teluguglobal'  # Update with your CSV folder path
# Directory to save the text files
corpus_directory = '/home/llmtelugu/data/teluguglobal'  # Update with your text data folder path

# Provide the path for the failed.csv file
failed_csv_path = '/home/llmtelugu/code/teluguglobal/failed_urls.csv'
