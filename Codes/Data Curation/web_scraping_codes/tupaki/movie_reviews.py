...
def extract_links(html_content, base_url):
    ...
    main_content_div = soup.find("div", class_="entertainment-bg")
    if main_content_div:
        rows = main_content_div.find_all("div", class_="entertainment-scroll")
        for row in rows:
...
def process_page(page_num, base_url, corpus_dir, link_base_url):
    if page_num == 1:
        ...
        url = f"{base_url}/{page_num}?pageUrl=/movies-reviews&content_count=4"
...
def main(base_url, corpus_dir, num_pages, max_workers=10):
    failed_urls = []
    link_base_url = "https://www.tupaki.com"

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_page = {executor.submit(process_page, page_num, base_url, corpus_dir, link_base_url): page_num for page_num in range(1, num_pages + 1)}
        ...
        for future in as_completed(future_to_page):
            ...
            try:
                ...
                failed_urls.append(f"{base_url}/{page_num}?pageUrl=/movies-reviews&content_count=4")
...
# Parameters
base_url = "https://www.tupaki.com/movies-reviews"
corpus_dir = '/home/llmtelugu/csv_data/tupaki/movie-reviews'
num_pages = 198
