...
#                 retries += 1
#                 time.sleep(retry_interval + random.uniform(0, 2))  # Add a small random delay
...
# csv_folder_path = '/home/llmtelugu/csv_data/telugu_mirchi'  # Update with your CSV folder path
# # Directory to save the text files
# corpus_directory = '/home/llmtelugu/data/telugu_mirchi'  # Update with your text data folder path

# # Provide the path for the failed.csv file
# failed_csv_path = '/home/llmtelugu/csv_data/telugu_mirchi/failed_urls.csv'
...
def crawl_data_from_link_with_retry(link, max_retries=3, retry_interval=5):
    ...
    while retries < max_retries:
        try:
            ...
            print(f"An error occurred while fetching data from {link}: {e}. Retrying...")
        retries += 1
        time.sleep(retry_interval)  # Adjusted the delay to a constant value
    print(f"Failed to fetch data from {link} after {max_retries} retries.")
    return None
    ...
    for category_folder in category_folders:
        ...
        with open(failed_csv_path, 'w', newline='', encoding='utf-8') as failed_csv:
            ...
            for csv_file_name in csv_files:
                ...
                with ThreadPoolExecutor(max_workers=20) as executor:  # Increased max_workers for better concurrency
                    future_to_link = {executor.submit(process_link_and_write, link, text_file_path): link for link in links}
                    for future in as_completed(future_to_link):
                        link = future_to_link[future]
                        try:
