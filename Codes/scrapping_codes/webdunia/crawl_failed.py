...
def extract_data_from_html(html_content):
    ...
    try:
        ...
        div_elements = soup.find_all('div', style="text-align: justify;")

        for div in div_elements:
            div_text = div.get_text(separator=' ').strip()
            telugu_text = [text.strip() for text in div_text.split() if is_telugu(text)]
...
# Function to process a single link and append data to text files
def process_link(link, text_file, append_text_file):
    html_content = crawl_data_from_link(link)
    if html_content:
        extracted_data = extract_data_from_html(html_content)
        if extracted_data:
            print(f"Writing data from {link} into {text_file} and {append_text_file}\n")
            with open(text_file, 'a', encoding='utf-8') as f:
                f.write('\n'.join(extracted_data) + '\n\n')  # Add an extra newline character

            with open(append_text_file, 'a', encoding='utf-8') as f:
                f.write('\n'.join(extracted_data) + '\n\n')  # Add an extra newline character
...
def process_failed_csv_file(file_path, corpus_directory, append_text_file, encoding='utf-8'):
    ...
    for idx, link in enumerate(links[1:]):
        ...
        success = process_link(link, text_file_path, append_text_file)
        if not success:
            print(f"Failed URL {idx + 1}/{len(links)}: {link}")
...
def process_all_failed_csv_files(failed_links_directory, corpus_directory, append_text_file, encoding='utf-8'):
    with ThreadPoolExecutor() as executor:
        ...
        for filename in os.listdir(failed_links_directory):
            if filename.startswith('failed_links_') and filename.endswith('.csv'):
                ...
                futures.append(executor.submit(process_failed_csv_file, file_path, corpus_directory, append_text_file, encoding))
...
# Directories
failed_links_directory = '/home/llmtelugu/data/webdunia/Debug'
corpus_directory = '/home/llmtelugu/data/webdunia/text_data'
append_text_file = '/home/llmtelugu/data/webdunia/text_data/combined_failed_text_data_3.txt'
encoding = 'utf-8'

# Process all failed CSV files
process_all_failed_csv_files(failed_links_directory, corpus_directory, append_text_file, encoding)
